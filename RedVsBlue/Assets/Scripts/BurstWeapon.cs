...
public class BurstWeapon : Weapon
{
    ...
    public override void Fire(Vector3 pos, bool faction1)
    {
        if (fireCooldown <= 0)
        {
            fireCooldown = firerate;
            StartCoroutine(FireProjectiles(pos, faction1));
        }
    }



    IEnumerator FireProjectiles(Vector3 pos, bool faction1)
    {
        Vector2 rot = (pos - this.transform.position);
        Vector3 spawnPos = (rot.normalized * Vector3.one);
        for (uint i = 0; i < burstCount; i++)
        {
